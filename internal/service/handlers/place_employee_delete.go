...
func PlaceEmployeeDelete(w http.ResponseWriter, r *http.Request) {
	...
	if len(employees) != 1 {
		if len(employees) == 0 {
			...
			log.Errorf("More than one employee found %s %s %s", placeId, userForDeleteId, employees)
			httpkit.RenderErr(w, problems.InternalError())
		}
		return
	}
	...
	employees, err = Server.MongoDB.PlacesEmployees.FilterByPlaceId(placeId).FilterByUserId(userForDeleteId).Get(r.Context())
	if err != nil {
	...
	delRole, err := roles.StringToRole(deletedUser.Role)
	if err != nil {
		log.Errorf("Failed to parse role: %v", err)
		httpkit.RenderErr(w, problems.BadRequest(err)...)
		return
	...
	if roles.CompareRoles(curRole, delRole) == 0 {
	...
	_, err = Server.MongoDB.PlacesEmployees.FilterById(deletedUser.ID).Delete(r.Context())
	if err != nil {
